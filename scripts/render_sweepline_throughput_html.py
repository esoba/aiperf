...
"""Render an interactive HTML view for AIPerf sweep-line throughput analysis."""
...
def build_html(
    ...
    credits_pending = ts.credits_pending[z]
    ...
    fig.add_trace(
        go.Scatter(
            ...
            fill="tozeroy",
            fillcolor="rgba(255, 93, 115, 0.28)",
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="t=%{x:.0f}s<br>credits pending=%{y:,.0f}<extra></extra>",
        ),
        row=2,
        col=1,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--title", default="Sweep-Line Throughput", help="Main title")
...
def main() -> int:
    ...
    output = args.output or args.input.with_name(
        args.input.stem + "-sweepline-throughput.html"
    )
