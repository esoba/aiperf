...
class DatasetMetadata(AIPerfBaseModel):
    ...
    default_threading_mode: TurnThreadingMode | None = Field(
        default=None,
        description="Dataset-level default for how prior turns are accumulated. "
        "Set by the loader based on dataset format semantics. "
        "Individual conversations can override this via their own turn_threading_mode field.",
    )
...
class Conversation(AIPerfBaseModel):
    ...
    turn_threading_mode: TurnThreadingMode | None = Field(
        default=None,
