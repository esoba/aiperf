...
def _extract_metric_labels(info_labels: InfoLabels | None) -> InfoLabels:
    """Extract metric-level labels for Prometheus measurement metrics.

    Only includes low-cardinality, server-identity labels (model, endpoint_type).
    Per-run parameters (benchmark_id, concurrency, streaming, request_rate) are
    excluded to prevent cardinality explosion and placed on aiperf_info only.
    Grafana/PromQL users join via aiperf_info for full context.
    """
    _INCLUDED = {"model", "concurrency"}
    return {k: v for k, v in (info_labels or {}).items() if k in _INCLUDED}
