...
class JobProgress(AIPerfBaseModel):
    ...
    def is_complete(self) -> bool:
        """Check if the profiling phase has completed."""
        profiling = self.phases.get(CreditPhase.PROFILING)
        return profiling is not None and profiling.is_requests_complete

    @property
    def profiling_stats(self) -> CombinedPhaseStats | None:
        """Get the profiling phase stats (primary benchmark phase)."""
        return self.phases.get(CreditPhase.PROFILING)

    @property
    def warmup_stats(self) -> CombinedPhaseStats | None:
        """Get the warmup phase stats."""
        return self.phases.get(CreditPhase.WARMUP)
                ...
                async with self._session.get(url) as response:
                    ...
                    if response.status in RETRYABLE_STATUS_CODES:
                        if attempt < self._max_retries:
                            logger.debug(
                                f"Retryable status {response.status} from {url}, "
                            ...
                            logger.warning(
                                ...
                                f"attempts with status {response.status}"
                            )
                            return None
