...
class WorkerPodManager(BaseComponentService):
    ...
    async def _download_dataset(self) -> tuple[Path, Path]:
        ...
        """
        if not self.service_config.dataset_api_base_url:
            raise RuntimeError(
                "No dataset_api_base_url configured. "
                "WorkerPodManager requires this to download the dataset."
            )

        base_url = self.service_config.dataset_api_base_url.rstrip("/")
    ...
    async def _prefetch_tokenizers(self) -> None:
        ...
        event loop. The resolved names are stored on the user config so
        ...
        resolved = await asyncio.to_thread(
            validate_tokenizer_early, self.user_config, logger
        )
        if resolved:
            self.user_config.tokenizer.resolved_names = resolved
    ...
    def _get_upload_base_url(self) -> str | None:
        ...
        base_url = self.service_config.dataset_api_base_url
        if not base_url:
            return None
