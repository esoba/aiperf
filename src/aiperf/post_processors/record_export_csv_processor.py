        ...
        **kwargs,
        ...
        if export_level not in (ExportLevel.RECORDS, ExportLevel.RAW):
            raise PostProcessorDisabled(
                f"CSV record export processor is disabled for export level {export_level}"
            )
        if RecordExportFormat.CSV not in user_config.output.record_export_formats:
            raise PostProcessorDisabled(
                "CSV record export disabled: format not selected in --record-export-formats"
            )

        output_file = user_config.output.profile_export_records_csv_file
