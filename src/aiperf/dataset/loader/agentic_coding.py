        ...
        for conv_id, entries in data.items():
            ...
            for entry in entries:
                ...
                if msg_count < prev_msg_count:
                    raise ValueError(
                        f"Trajectory {conv_id!r} at idx {entry.conversation_idx}: "
                        f"message count decreased from {prev_msg_count} to {msg_count}. "
                        f"Cumulative message history must be non-decreasing."
                    )
