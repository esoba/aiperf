...
# Cluster runtime: "kind" (default) or "minikube".
# Kind supports GPU via nvidia-ctk + device plugin inside the node.
CLUSTER_RUNTIME = os.environ.get("CLUSTER_RUNTIME", "kind").lower()


def _effective_runtime() -> str:
    """Return the active cluster runtime ('kind' or 'minikube').

    Defaults to 'kind' for both CPU and GPU systems.  Kind GPU support
    requires nvidia-ctk and Docker configured with nvidia as default runtime.
    Set CLUSTER_RUNTIME=minikube to use Minikube instead.
    """
...
_BANNER = (
    ...
    " Kind · Minikube · Docker · GPU · Mock · vLLM · Dynamo\n"
)
# fmt: on
...
Environment variables:
  ...
  CLUSTER_RUNTIME    kind|minikube (default: kind)
...
def _install_nvidia_in_kind_node() -> None:
    ...
    for _i in range(30):
        r = node_exec("crictl", "info", capture=True, check=False)
        if r.returncode == 0:
            break
        time.sleep(1)
    else:
        log_error("containerd failed to restart inside Kind node")
        raise SystemExit(1)
    ...
    for i in range(60):
        ...
        log_error("Node did not become Ready within 120s")
        raise SystemExit(1)
...
def _install_device_plugin() -> None:
    ...
    for i in range(60):
        ...
        try:
            if int(r.stdout.strip()) > 0:
                log_success(f"nvidia.com/gpu: {r.stdout.strip()}")
                return
    ...
    log_error("nvidia.com/gpu not found in node allocatable after 120s")
    kubectl(
        "-n",
        "kube-system",
        "logs",
...
def cmd_cluster_create() -> str:
    ...
    return _minikube_cluster_create()
...
def cmd_install_dynamo() -> str:
    ...
    try:
        ...
        if os.path.exists(platform_tgz):
...
def cmd_status() -> None:
    ...
    print()
    rt = _effective_runtime()
    title = f"AIPerf {rt.capitalize()} Cluster Status"
    print("\u2554" + "\u2550" * 60 + "\u2557")
    ...
    if cluster_running():
        ...
        if rt == "kind":
            print("\nRun: ./dev/kube.py cluster-create")
        else:
    ...
    if r.returncode == 0 and r.stdout.strip():
        ...
        detected_mode = DYNAMO_MODE
        for known in ("disagg-1gpu", "disagg", "agg"):
            if any(f"dynamo-{known}" in line for line in lines):
                detected_mode = known
                break
        print(
            _ok(
                f"Deployed ({running}/{len(lines)} pods running) — mode: {detected_mode}"
            )
        )
    else:
