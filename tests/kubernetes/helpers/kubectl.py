...
class KubectlClient:
    ...
    async def create_namespace(self, namespace: str) -> None:
        ...
        Args:
            namespace: Namespace name.
        """
        await self.run("create", "namespace", namespace, check=True)
    ...
    async def delete_namespace(self, namespace: str, wait: bool = True) -> None:
        ...
        if wait:
            ...
            logger.warning(f"Namespace {namespace} still exists after 60s")
    ...
    async def wait_for_benchmark_api(
        ...
        async with self.port_forward(pod, api_port, namespace=namespace) as local_port:
