...
class AIPerfJobConfig:
    ...
    def to_cr_manifest(self, name: str, namespace: str) -> str:
        """Generate AIPerfJob CR manifest.
...
class OperatorDeployer:
    ...
    async def install_crd(self) -> None:
        """Install the AIPerfJob CRD."""
        crd_path = self.project_root / "deploy" / "crd.yaml"
        logger.info(f"Installing CRD from {crd_path}")
    ...
    async def deploy_operator(self) -> None:
        ...
        """
        operator_path = self.project_root / "deploy" / "operator.yaml"
        logger.info(f"Deploying operator from {operator_path}")
        ...
        manifest = manifest.replace(
            "image: aiperf-operator:latest",
            f"image: {self.operator_image}",
        )
        manifest = manifest.replace(
    ...
    async def cleanup_all(self) -> None:
        """Clean up all deployed jobs."""
        for result in self._deployed_jobs:
            await self.cleanup_job(result)
