...
class TestOperatorCancellation:
    ...
    @pytest.mark.skip(
        reason="CRD does not define spec.cancel field - cancellation not functional"
    )
    @pytest.mark.timeout(TEST_JOB_TIMEOUT)
    @pytest.mark.asyncio
    async def test_cancel_running_job(
        ...
        NOTE: This test is skipped because the CRD doesn't define the spec.cancel
        field. The cancel handler exists in the operator but cannot be triggered
        without the field in the CRD schema.
