...
@pytest.mark.skip(
    reason="BenchmarkCompleteMessage handler removed; _benchmark_complete now set by _on_process_records_result in ResultsRouter"
)
class TestBenchmarkCompleteHandler:
    ...
    async def test_on_benchmark_complete_sets_flag(
        ...
        await mock_fastapi_service._on_benchmark_complete(message)

        assert mock_fastapi_service._routers["results"]._benchmark_complete is True
        mock_fastapi_service.info.assert_called()
