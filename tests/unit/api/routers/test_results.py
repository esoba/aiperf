...
def make_profile_results(
    ...
    records_dict = {r.tag: r for r in records}
...
class TestResultsEndpoint:
    ...
    def test_results_contains_metric_records(
        ...
        assert "latency" in records
        assert records["latency"]["avg"] == 150.0
        assert records["latency"]["p95"] == 200.0
        assert records["latency"]["p99"] == 250.0
...
class TestFinalResultsHandler:
    ...
    @pytest.mark.asyncio
    async def test_on_process_all_results_stores_results(
    ...
    @pytest.mark.asyncio
    async def test_on_process_all_results_replaces_previous(
        self, results_router: ResultsRouter
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_on_process_all_results_various_states(
        self,
        results_router: ResultsRouter,
        completed: int,
        was_cancelled: bool,
