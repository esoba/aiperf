...
class TestPromptGeneratorComprehensive:
    ...
    # ============================================================================
    # HashIdRandomGenerator Integration Tests
    # ============================================================================

    def test_hash_id_rng_initialized(self, basic_config):
        """Test that HashIdRandomGenerator is initialized in PromptGenerator."""
        from aiperf.common.hash_id_random_generator import HashIdRandomGenerator

        tokenizer, config = basic_config
        generator = PromptGenerator(config, tokenizer)

        assert hasattr(generator, "_hash_id_corpus_rng")
        assert isinstance(generator._hash_id_corpus_rng, HashIdRandomGenerator)

    def test_generate_cached_prompt_deterministic_per_hash_id(self, basic_config):
        """Test that same hash_ids produce identical prompts across calls."""
        tokenizer, config = basic_config
        generator = PromptGenerator(config, tokenizer)

        result1 = generator._generate_cached_prompt(10, [1, 2], 5)

        # Clear block cache to force regeneration
        generator._cache.clear()

        result2 = generator._generate_cached_prompt(10, [1, 2], 5)

        # Same hash_ids should produce identical results due to HashIdRandomGenerator
