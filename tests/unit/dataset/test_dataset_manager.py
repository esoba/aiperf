...
class TestDatasetManagerTokenizerSkip:
    ...
    @pytest.mark.asyncio
    @pytest.mark.usefixtures("_mock_dataset_steps")
    async def test_tokenizer_loaded_when_explicitly_set_on_non_tokenizing_endpoint(
        self, mock_tokenizer
    ):
        """Test that tokenizer is loaded when --tokenizer is explicitly set, even for non-tokenizing endpoint."""
        user_config = UserConfig(
            endpoint=EndpointConfig(
                model_names=["nvidia/nemoretriever-page-elements-v3"],
                type="image_retrieval",
