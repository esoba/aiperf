...
class TestAgenticCodingConvertToConversations:
    ...
    def test_rejects_decreasing_message_count(self, default_user_config):
        """Message count must be non-decreasing across entries (cumulative history)."""
        data = {
            "traj-001": [
                AgenticCodingEntry(
                    conversation_id="traj-001",
                ...
                AgenticCodingEntry(
                    conversation_id="traj-001",
                    conversation_idx=1,
                    messages=[{"role": "user", "content": "hi"}],  # fewer messages
                ),
            ]
        }
        loader = AgenticCodingDatasetLoader(
        ...
        with pytest.raises(ValueError, match="message count decreased"):
