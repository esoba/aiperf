...
class TestJobProgress:
    ...
    def test_is_complete_profiling_complete(self) -> None:
        """Test is_complete returns True when profiling is complete."""
        from aiperf.common.mixins.progress_tracker_mixin import CombinedPhaseStats
        ...
        progress = JobProgress(
            phases={
                CreditPhase.PROFILING: CombinedPhaseStats(
                    ...
                    total_expected_requests=100,
                    requests_end_ns=1000000000,  # Required to make is_requests_complete True
                )
            }
        )
    ...
    def test_profiling_stats_property(self) -> None:
        ...
        progress = JobProgress(phases={CreditPhase.PROFILING: stats})
    ...
    def test_warmup_stats_property(self) -> None:
        ...
        progress = JobProgress(phases={CreditPhase.WARMUP: stats})
...
class TestProgressClientGetProgress:
    ...
    async def test_get_progress_success(
        ...
        with patch("aiohttp.ClientSession") as mock_session_class:
            ...
            assert progress.current_phase == CreditPhase.PROFILING
            assert CreditPhase.PROFILING in progress.phases
            assert CreditPhase.WARMUP in progress.phases
...
class TestProgressClientParseResponse:
    ...
    def test_parse_with_phases(
        ...
        assert CreditPhase.PROFILING in progress.phases
        assert CreditPhase.WARMUP in progress.phases
        assert progress.phases[CreditPhase.PROFILING].requests_completed == 250
    ...
    def test_parse_invalid_phase_ignored(self) -> None:
        ...
        assert "invalid_phase" not in [p.value for p in progress.phases]
        assert CreditPhase.PROFILING in progress.phases
...
class TestRequestWithRetry:
    ...
    async def test_exponential_backoff_increases_delay(self) -> None:
        ...
        with (
            ...
            # First sleep should be initial backoff
            assert sleep_times[0] == 0.5
            # Second sleep should be 2x
            assert sleep_times[1] == 1.0
            # Third sleep should be 4x
            assert sleep_times[2] == 2.0
