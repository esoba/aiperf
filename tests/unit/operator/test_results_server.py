...
class TestDashboardMount:
    """Tests for Dash dashboard mounted at /dashboard/."""

    @pytest.fixture
    def results_dir_with_runs(self, tmp_path: Path) -> Path:
        """Create a results dir with two valid zst runs."""
        cctx = zstandard.ZstdCompressor()
        for job in ("job-1", "job-2"):
            d = tmp_path / "default" / job
            ...
            (d / "profile_export.jsonl.zst").write_bytes(
            ...
            (d / "profile_export_aiperf.json.zst").write_bytes(cctx.compress(agg))
        return tmp_path

    @pytest.mark.asyncio
    async def test_dashboard_endpoint_responds(
        self, results_dir_with_runs: Path
    ) -> None:
        """GET /dashboard/ returns 200 when runs exist."""
        app = create_app(results_dir=results_dir_with_runs)
        ...
        try:
            ...
            await ctx.__aexit__(None, None, None)
        assert resp.status_code == 200
        assert "AIPerf Dashboard" in resp.text

    @pytest.mark.asyncio
    async def test_dashboard_refresh(self, results_dir_with_runs: Path) -> None:
        """POST /api/v1/dashboard/refresh rescans PVC."""
        app = create_app(results_dir=results_dir_with_runs)
        async with httpx.AsyncClient(
            transport=httpx.ASGITransport(app=app), base_url="http://test"
        ) as client:
            resp = await client.post("/api/v1/dashboard/refresh")
        assert resp.status_code == 200
        data = resp.json()
        assert data["runs_found"] == 2
