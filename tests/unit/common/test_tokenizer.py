...
from aiperf.common.tokenizer import Tokenizer, _is_network_error
...
class TestTokenizerRetry:
    ...
    async def test_successful_load_no_retry(self):
        ...
        mock_tokenizer._resolved_name = "test-model"
        ...
        with patch(
            "aiperf.common.tokenizer.asyncio.to_thread", side_effect=mock_to_thread
        ):
            result = await Tokenizer.from_pretrained("test-model")
    ...
    @pytest.mark.asyncio
    async def test_retry_on_network_error_then_success(self):
        ...
        network_error = TokenizerError(
            "Failed to load tokenizer 'test-model'", tokenizer_name="test-model"
        )
        network_error.__cause__ = Exception("Connection timeout")
        ...
        with (
            ...
            patch("aiperf.common.tokenizer.asyncio.sleep", new=AsyncMock()),
        ):
            result = await Tokenizer.from_pretrained("test-model")
    ...
    async def test_exhaust_all_retries(self):
        ...
        with (
            ...
            with pytest.raises(TokenizerError):
                await Tokenizer.from_pretrained("test-model")

            # Should try 4 times (initial + 3 retries)
            assert call_count == 4

    @pytest.mark.asyncio
    async def test_no_retry_on_non_network_error(self):
        """Non-network errors should fail immediately without retry."""
        non_network_error = TokenizerError(
            "Failed to load tokenizer 'test-model'", tokenizer_name="test-model"
        )
        non_network_error.__cause__ = ValueError("Invalid model name")
    ...
    async def test_exponential_backoff_delays(self):
        ...
        with (
            ...
            patch("aiperf.common.tokenizer.asyncio.sleep", side_effect=mock_sleep),
            patch("aiperf.common.tokenizer.Environment.TOKENIZER.INIT_MAX_RETRIES", 3),
            patch(
                "aiperf.common.tokenizer.Environment.TOKENIZER.INIT_BASE_DELAY",
                1.0,
            ),
        ):
            with pytest.raises(TokenizerError):
    ...
    async def test_custom_retry_configuration(self):
        ...
        network_error.__cause__ = Exception("HTTP 429")
