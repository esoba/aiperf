...
class TestValidateTokenizerEarly:
    ...
    def base_user_config(self):
        """Minimal user config with fields needed by validate_tokenizer_early."""
        config = MagicMock()
        config.endpoint.use_server_token_count = False
        config.endpoint.model_names = ["model-a"]
    ...
    def test_skips_when_server_token_count_with_real_data(
        ...
        base_user_config.input.file = "/some/file.jsonl"
    ...
    def test_returns_mapping_from_model_names(
        ...
        base_user_config.endpoint.model_names = ["model-a", "model-b"]
