...
class TestKubernetesDeploymentKueue:
    ...
    def test_get_jobset_spec_passes_queue_name(
        ...
        deployment = KubernetesDeployment(**_deployment_kwargs, queue_name="team-queue")
        js_spec = deployment.get_jobset_spec()
        assert js_spec.queue_name == "team-queue"

    def test_get_jobset_spec_passes_priority_class(
        self, _deployment_kwargs: dict[str, Any]
    ) -> None:
        deployment = KubernetesDeployment(
            **_deployment_kwargs, priority_class="high-priority"
        )
        js_spec = deployment.get_jobset_spec()
        assert js_spec.priority_class == "high-priority"
    ...
    def test_get_jobset_spec_passes_none_by_default(
        ...
        js_spec = deployment.get_jobset_spec()
        assert js_spec.queue_name is None
        assert js_spec.priority_class is None
    ...
    def test_get_jobset_spec_both_fields_propagate(
        ...
        js_spec = deployment.get_jobset_spec()
        assert js_spec.queue_name == "shared-queue"
        assert js_spec.priority_class == "low"
