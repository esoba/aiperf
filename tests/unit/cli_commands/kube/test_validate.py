...
class TestValidateK8sName:
    ...
    def test_single_char_name_no_match(self) -> None:
        """Single char doesn't match pattern [a-z0-9][a-z0-9-]*[a-z0-9] (needs >= 2)."""
        result = ValidationResult(path=Path("test.yaml"))
        validate_k8s_name("a", result)
        assert any("not a valid Kubernetes resource name" in e for e in result.errors)
...
class TestValidateFile:
    ...
    def test_all_validations_run(self, tmp_path: Path) -> None:
        """A valid file exercises all validation stages without errors."""
        doc = _minimal_doc()
