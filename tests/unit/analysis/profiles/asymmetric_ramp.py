...
"""Asymmetric ramp: fast ramp-up (2%), long drain (30%)."""
...
def generate(rng: np.random.Generator, n: int = 10_000) -> SyntheticBenchmark:
    """Quick ramp-up, extended ramp-down simulating long completion tails."""
    total_dur = 100 * _NS
    ramp_end = 2 * _NS
    drain_start = 70 * _NS
    ...
    # Long drain: starts near drain_start, ends spread to total_dur
    drain_starts = rng.uniform(drain_start - 100e6, drain_start, n_drain)
    drain_latency = rng.exponential(100, n_drain) * 1e6 + 50e6
