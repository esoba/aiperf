...
When a turn has `raw_messages` set, those message dicts are expanded directly into the messages list. This is used by trace replay dataset types (e.g., `claude_code_trace`, `coding_trace`) for faithful reproduction of tool use conversations:
...
### Trace Replay Datasets

The `anthropic_messages` endpoint works with trace replay dataset types that produce `raw_messages` turns. These include `claude_code_trace` and `coding_trace`, which replay full tool-use conversations verbatim:
...
See the [Claude Code Trace Replay](claude-code-trace-replay.md) and [Coding Trace Replay](coding-trace-replay.md) tutorials for details.
