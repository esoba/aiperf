...
CLI commands defined in `src/aiperf/cli_commands/kube.py`:

| Command | Purpose |
|---------|---------|
| `profile` | Deploy and run benchmark |
| `generate` | Generate YAML manifests for GitOps |
| `attach` | Attach to running benchmark |
| `status` | Check job status |
| `logs` | Stream logs |
| `results` | Retrieve results |
| `delete` | Clean up resources |
| `cancel` | Stop running benchmark |
| `monitor` | Live dashboard monitoring |
| `preflight` | Validate cluster setup |

## 2. Deployment Generation

The runner in `src/aiperf/kubernetes/runner.py` orchestrates deployment:
...
|----------|---------|-------------|
| `AIPERF_K8S_CONTROLLER_CPU_REQUEST` | 600m | Controller CPU request |
| `AIPERF_K8S_CONTROLLER_MEMORY_LIMIT` | 4Gi | Controller memory limit |
| `AIPERF_K8S_WORKER_CPU_REQUEST` | 350m | Worker CPU request |
| `AIPERF_K8S_WORKER_MEMORY_LIMIT` | 2Gi | Worker memory limit |
